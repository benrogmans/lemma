...
pub fn api_v1_schema() -> Value {
    ...
    let defs = json!({
        ...
        "GraphQueryResponse": {
            "type": "object",
            "required": ["spec", "nodes", "edges", "truncated"],
            "additionalProperties": false,
            "description": "Result of Engine::graph_query: bounded semantic graph slice for one spec and effective instant.",
            "properties": {
                "spec": {"type": "string"},
    ...
    json!({
        ...
        "description": "Schema for the JSON document shapes the engine produces at its consumer boundary: Show (Engine::show), Response (Engine::run), GraphQueryResponse (Engine::graph_query), list (Engine::list, array of ResolvedRepository), errors (array of EngineError), and RepositoryInstallResult (SDK install). Explanation trees live under RuleResult.explanation (RuleNode / ExplanationNode).",
        "$defs": defs,
        "anyOf": [
            {"$ref": "#/$defs/Show"},
            {"$ref": "#/$defs/Response"},
            {"$ref": "#/$defs/GraphQueryResponse"},
